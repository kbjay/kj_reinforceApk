...
unsigned char key[] = {0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39};
...
unsigned char *des_encipher(unsigned char *plaintext, int l) {
    ///明文长度
    int nPlaintextLen = l;
    int nCiphertextLen = nPlaintextLen;

    int remainders = nPlaintextLen % 8;
    ///需要填充
    if (remainders) {
        ///密文长度，加了填充位
        nCiphertextLen = nCiphertextLen + (8 - remainders);
    }

    unsigned char *buf = new unsigned char(nCiphertextLen);
    memset(buf, 0, nCiphertextLen);

    ///填充'\0' nopadding
    unsigned char *temp = new unsigned char(nCiphertextLen);
    memset(temp, '\0', nCiphertextLen);
    memcpy(temp, plaintext, nPlaintextLen);

    for (int i = 0; i < nPlaintextLen; i += 8) {
        des_main(temp + i, buf + i, key, encipher);
    }
    return buf;
}
 ...
 * @return
 */
unsigned char *des_decipher(unsigned char *ciphertext, int l) {
    int nCiphertextLen = l;
    unsigned char *plaintext = new unsigned char(nCiphertextLen);
    for (int i = 0; ciphertext[i]; i += 8) {
        des_main(ciphertext + i, plaintext + i, key, decipher);
    }
    return plaintext;
}
...
void des_decryptFile(char *sourceFile, char *destFile) {
    ...
    std::ifstream is(sourceFile, std::ifstream::binary);
    std::ofstream os(destFile, std::ifstream::binary);
    ///获取文件长度
    ...
    ///解析前面的字节
    char *b = new char[len - 8];
    is.read(b, len - 8);
    unsigned char *result = des_decipher((unsigned char *) b, len - 8);
    os.write(reinterpret_cast<const char *>(result), len - 8);
    free(result);
    free(b);
    ...
    char *buf = new char[8];
    is.read(buf, 8);
    unsigned char *r = des_decipher((unsigned char *) buf, 8);
    int temp = 0, i;
    for (i = 0; i < 8; i++) {
        if (r[i] != '\0') {
            temp++;
    ...
    os.write(reinterpret_cast<const char *>(r), temp);
    free(r);
    free(buf);
    is.close();
    os.close();
//    Logd("start decrypt->source %s", sourceFile);
//    Logd("start decrypt->dest %s" , destFile);
//    std::ifstream is(sourceFile, std::ifstream::binary);
//    std::ofstream os(destFile, std::ifstream::binary);
//    ///获取文件长度
//    is.seekg(0, std::ios_base::end);
...
//        is.read(buffer, 8);
//        Logd("start decrypt->ing %s" , buffer);
//        unsigned char *result = des_decipher((unsigned char *) buffer, 8);
//        if (i == len / 8 - 1) {
//            ///去掉'\0'
//            int temp = 0, i;
//            for (i = 0; i < 8; i++) {
...
//                }
//            }
//            os.write(reinterpret_cast<const char *>(result), temp);
//        } else {
//            os.write(reinterpret_cast<const char *>(result), 8);
//        }
//        free(buffer);
//    }
//    Logd("end decrypt");
...
void des_encryptFile(char *sourceFile, char *destFile) {
    ...
    for (i = 0; i < len / 8; i++) {
        ...
        unsigned char *result = des_encipher((unsigned char *) buffer, 8);
        os.write(reinterpret_cast<const char *>(result), 8);
    }

    if (len % 8 != 0) {
        char *buffer = new char[len % 8];
        is.read(buffer, len % 8);
        std::cout << buffer << std::endl;
        unsigned char *result = des_encipher((unsigned char *) buffer, len % 8);
        os.write(reinterpret_cast<const char *>(result), 8);
    }
